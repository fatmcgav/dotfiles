require("hs.hotkey").setLogLevel("verbose")
hs_log_level = hs.hotkey.getLogLevel()
logger.f("Hotkey log level = %s", hs_log_level)
...
hs.fnutils.each(grid, function(entry)
  ...
  bindKey(entry.key, function()
    ...
    -- logger.df("Units looks like: %s", printTable(units))
    logger.df("Window = %s, Geo = %s", window, windowGeo)

    local index = 0
    logger.df("Index = %i", index)
    hs.fnutils.find(units, function(unit)
      index = index + 1
      logger.df("Inside fnutils.find. Index = %i", index)
      logger.df("Unit looks like: %s", unit)
      printTable(unit)

      local geo = screen:fromUnitRect(unit)
      logger.df("Geo looks like: %s", geo)
      -- sometimes the screen reports slightly larger than the usable space
      local geoWithWiggle = hs.geometry.copy(geo)
      geoWithWiggle.h = geoWithWiggle.h - 7
      geoWithWiggle.w = geoWithWiggle.w - 4
      logger.df("Geo with wiggle = %s", geoWithWiggle)
      return windowGeo:equals(geo) or windowGeo:equals(geoWithWiggle)
    end)
    if index == #units then index = 0 end
    logger.df("Outside fnutils.find. Index = %i", index)
    logger.df("Attempting to move to: %s", units[index + 1])
